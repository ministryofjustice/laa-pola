# Ministry of Justice LAA Provider Office LookUp API (POLA)
...
## LAA Provider Office LookUp API (POLA)


The Provider Office Lookup API (POLA) is an API that retrieves legal provider contact information directly from the PDA in real time. FALA utilises POLA to access up-to-date provider office locations and contact details, benefiting from the automated data updates and queries without the need for human intervention.

POLA is designed for users who require access to legal provider contact information, making it an essential tool for anyone needing accurate and up-to-date provider details. Additionally, POLA is ideal for users who need to identify legal providers closest to their geographic location, leveraging geocoding technology to deliver precise results.
